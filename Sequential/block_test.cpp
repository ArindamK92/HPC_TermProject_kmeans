...
void calculateDistance(double* A, double* Cn, double* D, int n, int m, int k, int Block) {
    
    for (int l = 0; l < k; l+=Block) {
        for (int i = 0; i < n; i+=Block) {
            for (int j = 0; j < m; j+=Block) {
		    for (int l1 = l; l1 < l+Block; l1++) {
			    for (int i1 = i; i1 < i+Block; i1++) {
				    for (int j1 = j; j1 < j+Block; j1++) {

               				 D[i1 * k + l1] += (A[i1 * m + j1] - Cn[l1 * m + j1])*(A[i1 * m + j1] - Cn[l1 * m + j1]);
           				 }
       				    }
   			    }
		     }
...
int main(int argc, char** argv) {
    ...
    int Block = atoi(argv[5]); //block size
...
//    printMatrix(Cn, k, m);
    ...
    timer.start();
    for (int i = 0; i < itr; i++) {
        calculateDistance(A, Cn, D, n, m, k, Block);
